...
```{r}
zosteraDEG <- read.delim("../EdgeR/DE.EXP.CON.FDR.Z.Annot.txt") #Import Z. marina differentially expressed genes from edgeR
head(zosteraDEG) #Confirm import
zosteraDEG <- data.frame("geneIDs" = zosteraDEG$GeneID,
...
```{r}
zosteraGeneAnnot <- left_join(zosteraGeneAnnot, zosteraDEG) #Add edgeR information to annotation table
...
sum(is.na(blackGeneAnnot$edgeR.pvalue) == "FALSE") #Number of DEG = 485
```
