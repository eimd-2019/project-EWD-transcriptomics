...
```{r}
zosteraDEG <- read.delim("../EdgeR/EXP.CON.Zostera.ALL.txt") #Import Z. marina edgeR results (all genes, not just DEG)
head(zosteraDEG) #Confirm import
zosteraDEG <- data.frame("geneIDs" = rownames(zosteraDEG),
                         "logFC" = zosteraDEG$logFC,
                         "logCPM" = zosteraDEG$logCPM,
                         "FDR" = zosteraDEG$FDR,
                         "edgeR.pvalue" = zosteraDEG$PValue) #Only save gene ID, log fold change, log counts per million, FDR, and p-values from edgeR
head(zosteraDEG) #Confirm changes
```

```{r}
zosteraGeneAnnot <- left_join(zosteraGeneAnnot, zosteraDEG, "geneIDs") #Add edgeR information to annotation table
...
```{r}
write.table(geneInfo, "zosteraGeneList-WGCNA-MM-GS-Annot.txt", sep = "\t", col.names = TRUE, row.names = TRUE, quote = FALSE, na = "") #Save master table
```

```{r}
geneInfo <- read.delim("zosteraGeneList-WGCNA-MM-GS-Annot.txt", header = TRUE, sep = "\t", na.strings = "") #Import table using "" as NA
head(geneInfo) #Confirm import
```

```{r}
write.table(geneInfo, "zosteraGeneList-WGCNA-MM-GS-Annot.txt", sep = "\t", col.names = TRUE, row.names = TRUE, quote = FALSE) #Save master table again, but with NA instead of blank cells
```
...
```{r}
blackGeneAnnot <- cbind(rownames(geneInfo), geneInfo) #Create new dataframe for black module gene annotations and include rownames (trinity IDs) as a separate column
blackGeneAnnot <- blackGeneAnnot %>% 
  filter(., moduleColor == "black") %>% 
  select(., 1:3, GOterms, logFC, logCPM, FDR, edgeR.pvalue) %>% 
  separate_rows(., GOterms, sep=";") #Subset genes from black module, retain annotation columns, then separate GOterms into different rows
colnames(blackGeneAnnot) <- c("geneID", "Uniprot", "annotation", "GOID", "logFC", "logCPM", "FDR", "edgeR.pvalue") #Revise column names
head(blackGeneAnnot) #Confirm formatting
```

```{r}
blackGeneAnnot %>% 
  filter(., edgeR.pvalue != "NA") %>%
  distinct(., geneID) %>%
  summarise(., length(geneID)) #Retain rows where there is an edgeR p-value, identify unique geneIDs, then count the number of unique geneIDs

#Number of DEG in module = 61
```


```{r}
blackGeneAnnotGOMWU <- left_join(blackMFGOMWUSig, blackGeneAnnot, by = "GOID") #Join by GOID
head(blackGeneAnnotGOMWU) #Confirm formatting
length(unique(blackGeneAnnotGOMWU$geneID)) #Number of unique genes associated with GOterms = 17
sum(is.na(blackGeneAnnotGOMWU$edgeR.pvalue) == "FALSE") #Number of DEG = 7
```

```{r}
write.csv(blackGeneAnnotGOMWU, "zostera-BlackModule-EnrichedGOterms-DEG.csv", quote = FALSE, row.names = FALSE) #Save as .csv
```
...
```{r}
blueGeneAnnot <- cbind(rownames(geneInfo), geneInfo) #Create new dataframe for blue module gene annotations and include rownames (trinity IDs) as a separate column
blueGeneAnnot <- blueGeneAnnot %>% 
  filter(., moduleColor == "blue") %>% 
  select(., 1:3, GOterms, logFC, logCPM, FDR, edgeR.pvalue) %>% 
  separate_rows(., GOterms, sep=";") #Subset genes from blue module, retain annotation columns, then separate GOterms into different rows
colnames(blueGeneAnnot) <- c("geneID", "Uniprot", "annotation", "GOID", "logFC", "logCPM", "FDR", "edgeR.pvalue") #Revise column names
head(blueGeneAnnot) #Confirm formatting
```
...
#Number of DEG in module = 355
```

```{r}
blueGeneAnnotGOMWU <- left_join(blueBPGOMWUSig, blueGeneAnnot, by = "GOID") #Join by GOID
head(blueGeneAnnotGOMWU) #Confirm formatting
length(unique(blueGeneAnnotGOMWU$geneID)) #Number of unique genes associated with GOterms = 1
sum(is.na(blueGeneAnnotGOMWU$edgeR.pvalue) == "FALSE") #Number of DEG = 0
```

```{r}
write.csv(blueGeneAnnotGOMWU, "zostera-BlueModule-EnrichedGOterms-DEG.csv", quote = FALSE, row.names = FALSE) #Save as .csv
```
