...
```{r}
#pdf("module-treatment-relationships.pdf", height = 11, width = 8.5)
...
### Count the number of differentially expressed genes in each significant module
...
nrow(brownGeneList) #Number of rows
head(brownGeneList) #Confirm import
```

```{r}
left_join(x = brownGeneList, y = zosteraDEG, by = "geneIDs") %>%
  filter(., edgeR.pvalue < 0.05 & abs(logFC) > 10) %>%
  nrow(.) #Number of DEGs in the brown module
```

#### Black (exposed)

```{r}
blackGeneList <- read.delim("zosteraGeneList-black.txt", header = FALSE, sep = "\t", col.names = "geneIDs") #Import list of genes
nrow(blackGeneList) #Number of rows
head(blackGeneList) #Confirm import
```

```{r}
left_join(x = blackGeneList, y = zosteraDEG, by = "geneIDs") %>%
  filter(., edgeR.pvalue < 0.05 & abs(logFC) > 10) %>%
  nrow(.) #Number of DEGs in the black module
```
...
nrow(blueGeneList) #Number of rows
head(blueGeneList) #Confirm import
```

```{r}
left_join(x = blueGeneList, y = zosteraDEG, by = "geneIDs") %>%
  filter(., edgeR.pvalue < 0.05 & abs(logFC) > 10) %>%
  nrow(.) #Number of DEGs in the blue module
```
...
nrow(yellowGeneList) #Number of rows
head(yellowGeneList) #Confirm import
```

```{r}
left_join(x = yellowGeneList, y = zosteraDEG, by = "geneIDs") %>%
  filter(., edgeR.pvalue < 0.05 & abs(logFC) > 10) %>%
  nrow(.) #Number of DEGs in the yellow module
```

#### Light green (control)

```{r}
lightgreenGeneList <- read.delim("zosteraGeneList-lightgreen.txt", header = FALSE, sep = "\t", col.names = "geneIDs") #Import list of genes
nrow(lightgreenGeneList) #Number of rows
head(lightgreenGeneList) #Confirm import
```

```{r}
left_join(x = lightgreenGeneList, y = zosteraDEG, by = "geneIDs") %>%
  filter(., edgeR.pvalue < 0.05 & abs(logFC) > 10) %>%
  nrow(.) #Number of DEGs in the lightgreen module
```
...
I'm also going to count the number of genes and DEG in each module.
...
geneInfo %>% 
  ...
  summarise(., length(moduleColor)) #Number of genes in the module = 241

geneInfo %>% 
  filter(., moduleColor == "brown") %>%
  filter(., edgeR.pvalue < 0.05) %>%
  summarise(., length(moduleColor)) #Number of DEG in the module = 144
```
...
geneInfo %>% 
  ...
  summarise(., length(moduleColor)) #Number of genes in the module = 132

geneInfo %>% 
  filter(., moduleColor == "black") %>%
  filter(., edgeR.pvalue < 0.05) %>%
  summarise(., length(moduleColor)) #Number of DEG in the module = 71
```
...
geneInfo %>% 
  ...
  summarise(., length(moduleColor)) #Number of genes in the module = 450

geneInfo %>% 
  filter(., moduleColor == "blue") %>%
  filter(., edgeR.pvalue < 0.05) %>%
  summarise(., length(moduleColor)) #Number of DEG in the module = 420
```
...
geneInfo %>% 
  ...
  summarise(., length(moduleColor)) #Number of genes in the module = 229

geneInfo %>% 
  filter(., moduleColor == "yellow") %>%
  filter(., edgeR.pvalue < 0.05) %>%
  summarise(., length(moduleColor)) #Number of DEG in the module = 38
```
...
```{r}
geneInfo %>% 
  filter(., moduleColor == "lightgreen") %>%
  summarise(., length(moduleColor)) #Number of genes in the module = 42

geneInfo %>% 
  filter(., moduleColor == "lightgreen") %>%
  filter(., edgeR.pvalue < 0.05) %>%
  summarise(., length(moduleColor)) #Number of DEG in the module = 20
```
