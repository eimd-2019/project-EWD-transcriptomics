...
COMBINE MATCHED GENE LIST WITH GO MWU LIST, THEN MATCH WITH ZOSTERA DEG (have gene ID in hand).
...
blueGeneAnnot %>% 
  filter(., edgeR.pvalue < 0.05) %>%
  distinct(., geneID) %>%
  summarise(., length(geneID)) #Retain rows where there is a significant edgeR p-value, identify unique geneIDs, then count the number of unique geneIDs

#Number of DEG in module = 420
```

```{r}
blueGeneAnnotGOMWU <- left_join(blueBPGOMWUSig, blueGeneAnnot, by = "GOID") #Join by GOID
head(blueGeneAnnotGOMWU) #Confirm formatting
length(unique(blueGeneAnnotGOMWU$geneID)) #Number of unique genes associated with GOterms = 17
sum(blueGeneAnnotGOMWU$edgeR.pvalue < 0.05) #Number of DEG = 9
```
