...
knitr::opts_knit$set(root.dir = normalizePath("../output/05-WGCNA/")) #Set root directory
```
...
```{r}
zosteraAllGenes <- read.delim("../04-edgeR/EXP.CON.Zostera.logCPM.txt", header = TRUE, quote = "") #Import data with log CPM information for all expressed genes
...
```{r}
#pdf("merged-cluster-dendrogram.pdf", height = 8.5, width = 11)
...
```{r}
#pdf("figures/module-treatment-relationships.pdf", width = 11, height = 8.5)
...
```{r}
#pdf("figures/module-treatment-relationships-NOTEXT.pdf", width = 11, height = 8.5)
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               cex.text = 0.5,
               zlim = c(-1,1),
               main = paste("Module-Treatment Relationships")) #Create a version without text for the multipanel plot
...
```{r}
zosteraEdgeROutput <- read.delim("../04-edgeR/EXP.CON.Zostera.ALL.txt") #Import Z. marina edgeR results (all genes, not just DEG)
...
left_join(x = brownGeneList, y = zosteraEdgeROutput, by = "geneIDs") %>%
  filter(., FDR < 0.05) %>%
  nrow(.) #194 DEGs in the brown module
```
...
left_join(x = yellowGeneList, y = zosteraEdgeROutput, by = "geneIDs") %>%
  filter(., FDR < 0.05) %>%
  nrow(.) #5 DEGs in the yellow module
```
...
left_join(x = magentaGeneList, y = zosteraEdgeROutput, by = "geneIDs") %>%
  filter(., FDR < 0.05) %>% 
  nrow(.) #60 DEGs in the magenta module
```

## Master list with membership and gene significance information

```{r}
zosteraGeneAnnot <- read.delim("../output/03-assembly-annotation/c-uniprot/Zostera-blast-annot-withGeneID-noIsoforms.tab", header = FALSE, sep = "\t") #Import gene annotation information
...
## Obtain common GOterms from each module

### brown

```{r}
brownGeneListAnnot <- geneInfo %>%
  rownames_to_column(., var = "geneIDs") %>%
  right_join(., y = brownGeneList, by = "geneIDs") %>%
  select(geneIDs, Uniprot, logFC, logCPM, edgeR.pvalue, FDR, BPGO, MFGO) #Annotate genes.
head(brownGeneListAnnot)
```

```{r}
write.table(brownGeneListAnnot, "zosteraGeneList-brown-annot.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save individual module list
```

```{r}
brownGeneListAnnot %>%
  separate_rows(., BPGO, sep = "; ") %>%
  group_by(BPGO) %>%
  summarise(count = n()) %>%
  arrange(., desc(count)) #Get GOterm counts for BP to identify common biological processes represented in the module
```

```{r}
brownGeneListAnnot %>%
  separate_rows(., MFGO, sep = "; ") %>%
  group_by(MFGO) %>%
  summarise(count = n()) %>%
  arrange(., desc(count)) #Get GOterm counts for MF to identify common biological processes represented in the module
```

### yellow

```{r}
yellowGeneListAnnot <- geneInfo %>%
  rownames_to_column(., var = "geneIDs") %>%
  right_join(., y = yellowGeneList, by = "geneIDs") %>%
  select(geneIDs, Uniprot, logFC, logCPM, edgeR.pvalue, FDR, BPGO, MFGO) #Annotate genes.
head(yellowGeneListAnnot)
```

```{r}
write.table(yellowGeneListAnnot, "zosteraGeneList-yellow-annot.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save individual module list
```
...
```{r}
blueGeneListAnnot <- geneInfo %>%
  rownames_to_column(., var = "geneIDs") %>%
  right_join(., y = blueGeneList, by = "geneIDs") %>%
  select(geneIDs, Uniprot, logFC, logCPM, edgeR.pvalue, FDR, BPGO, MFGO) #Annotate genes.
head(blueGeneListAnnot)
```

```{r}
write.table(blueGeneListAnnot, "zosteraGeneList-blue-annot.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save individual module list
```
...
```{r}
magentaGeneListAnnot <- geneInfo %>%
  rownames_to_column(., var = "geneIDs") %>%
  right_join(., y = magentaGeneList, by = "geneIDs") %>%
  select(geneIDs, Uniprot, logFC, logCPM, edgeR.pvalue, FDR, BPGO, MFGO) #Annotate genes.
head(magentaGeneListAnnot)
```

```{r}
write.table(magentaGeneListAnnot, "zosteraGeneList-magenta-annot.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save individual module list
```

```{r}
magentaGeneListAnnot %>%
  separate_rows(., BPGO, sep = "; ") %>%
  group_by(BPGO) %>%
  summarise(count = n()) %>%
  arrange(., desc(count)) #Get GOterm counts for BP to identify common biological processes represented in the module
...
ggsave("figures/brown-expression-boxplot.pdf", height = 8.5, width = 11)
```
...
ggsave("figures/yellow-expression-boxplot.pdf", height = 8.5, width = 11)
```
...
ggsave("figures/blue-expression-boxplot.pdf", height = 8.5, width = 11)
```
...
ggsave("figures/magenta-expression-boxplot.pdf", height = 8.5, width = 11)
```
...
#pdf("figures/multipanel-expression-boxplot.pdf", height = 8.5, width = 11)
...
zosteraGOAnnotations <- zosteraGeneAnnot %>%
  ...
  dplyr::rename(., GO = "GOterms") %>%
  mutate(GO = gsub(GO, pattern = " ", replacement = "")) #Create GO annotations table. Remove spaces bewteen ; delimiter and next GOterm
...
```{r}
input="brown-Table-of-Significance-Measures.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goAnnotations="GO-Annotations-Table.tab" # two-column, tab-delimited, one line per gene, multiple GO terms separated by semicolon. If you have multiple lines per gene, use nrify_GOtable.pl prior to running this script.
...
#Warning in wilcox.test.default(nrg[sgo.yes], nrg[sgo.no], alternative = Alternative) : cannot compute exact p-value with ties

#7 GO terms at 10% FDR
```

```{r}
read.delim("MWU_BP_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```

```{r}
results <- gomwuPlot(input, goAnnotations, goDivision,
          absValue=0.001,  #Specify absValue=0.001 if you are doing Fisher's exact test for standard GO enrichment or analyzing a WGCNA module (all non-zero genes = "good genes").
          ...
          colors=c("dodgerblue2","firebrick1","skyblue2","lightcoral") # these are default colors, un-remark and change if needed
)
```
...
pcut=0.1 # adjusted pvalue cutoff for representative GO
...
```{r}
brownBPGOs <- bestGOs #Save bestGOs as a new table
brownBPGOs
```
...
```{r}
read.delim("MWU_CC_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
read.delim("MWU_MF_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
brownMFGOs <- bestGOs #Save bestGOs as a new table
brownMFGOs
```
...
```{r}
input="yellow-Table-of-Significance-Measures.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goDivision="BP" # either MF, or BP, or CC
```
...
```{r}
input="blue-Table-of-Significance-Measures.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goDivision="BP" # either MF, or BP, or CC
```
...
```{r}
read.delim("MWU_CC_blue-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
read.delim("MWU_MF_blue-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
input="magenta-Table-of-Significance-Measures.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
goDivision="BP" # either MF, or BP, or CC
```
...
```{r}
read.delim("MWU_BP_magenta-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
read.delim("MWU_CC_magenta-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```
...
```{r}
read.delim("MWU_MF_magenta-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
  filter(p.adj < 0.1)
```

## Summarize GO-MWU results

```{r}
sigGOMWUResults <- rbind(read.delim("MWU_BP_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
                           filter(p.adj < 0.1),
                         read.delim("MWU_CC_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
                           filter(p.adj < 0.1),
                         read.delim("MWU_MF_brown-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
                         ...
                         read.delim("MWU_MF_magenta-Table-of-Significance-Measures.csv", sep = " ", header = TRUE) %>%
                           filter(p.adj < 0.1)) %>%
  mutate(module = c(rep("brown", times = 7),
                    rep("brown", times = 7),
                    rep("brown", times = 17),
                    "blue",
                    rep("blue", times = 2),
                    ...
                    rep("magenta", times = 2),
                    rep("magenta", times = 2)),
         cat = c(rep("Biological Process", times = 7),
                 rep("Cellular Component", times = 7),
                 rep("Molecular Function", times = 17),
                 "Cellular Component",
                 rep("Molecular Function", times = 2),
                 rep("Biological Process", times = 2),
                 rep("Cellular Component", times = 2),
                 rep("Molecular Function", times = 2))) %>%
  select(module, term, name, cat, p.adj, nseqs) #Create summary table
sigGOMWUResults
```

```{r}
write.csv(sigGOMWUResults, "significant-GOMWU-results.csv", quote = FALSE, row.names = FALSE) #Save table
```
